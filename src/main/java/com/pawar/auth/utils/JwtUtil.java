...
import com.pawar.auth.repository.RefreshTokensRepository;
import com.pawar.auth.repository.UserRepository;
import com.pawar.auth.service.UserService;
...
public class JwtUtil {
	...
	public String generateToken(UserDto userDto) throws ClientProtocolException, IOException, TokenNotFoundException {
		...
		if (!isTokenAlreadyAvailable) {
			...
			refreshTokens = refreshTokensRepository.findByUser_Id(loggerInUser.getUser_id()).get();
...
//		}
	...
	public Boolean isTokenAlreadyAvailable(User user) throws TokenNotFoundException {
		...
		Optional<RefreshTokens> refreshTokens = refreshTokensRepository.findByUser_Id(user.getUser_id());
